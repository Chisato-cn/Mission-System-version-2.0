...
namespace Tomoe.MissionSystem.Editor
{
    public class ConditionDrawer : VisualElement
    {
        ...
        {
            VisualTreeAsset visualTree = Resources.Load<VisualTreeAsset>("VisualTree/ConditionDrawer");
            visualTree.CloneTree(this);

            Undo.undoRedoPerformed += UndoRedoPerformed;
            RegisterCallback<DetachFromPanelEvent>(evt => Undo.undoRedoPerformed -= UndoRedoPerformed);
            
            SetupAndAddPolymorphicList("Condition", typeof(Condition), "conditions", "Conditions", property);
        }
        ...
        private ListView BindingListView(VisualElement root, SerializedProperty property, DerivedFromTypeSearchTree provider)
        {
            ...
            listview.headerTitle = "Conditions";
            SetupListView(listview, property, provider);
            
            return listview;
        }
        
        private void SetupAndAddPolymorphicList(string title, Type baseType, string propertyPath, string containerName, SerializedProperty property)
        {
            var searchTree = ScriptableObject.CreateInstance<DerivedFromTypeSearchTree>();
            searchTree.Init(title, baseType);
            
            var listView = BindingListView(this.Q<VisualElement>(containerName), property, searchTree);
